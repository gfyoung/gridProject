...
events_link = "https://graph.facebook.com/v2.0/129511477069092/events?access_token={}&fields=name,location,description"
    ...
    while True:
        ...
        for event in eventsInfo:
            if getStartDate(event) > today:
                upcomingEvents.append((event['name'], event['start_time'], \
        ...
        for event in eventsInfo:
            if getStartDate(event) <= today and \
               (today - getStartDate(event)).days <= daysAgo:
                pastEvents.append((event['name'], event['start_time'], \
                                   event['location'], event['description']))
...
def getVendorsForEvent(eventDescription, vendorList):
    pattern = re.compile('.*vendor.*:|.*truck.*:|.*lineup.*:', flags = re.IGNORECASE)
    ...
    pattern = re.compile('\W+.?truck.?|\W+.?vendor.?|\W+.?cart.?|\W+.?trailer.?', flags = re.IGNORECASE)
    eventVendors = []
